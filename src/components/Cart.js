...
const Cart  = ({ products, total, onCheckoutClicked, onRemoveItemClicked, onIncrementQtyClicked, onDecrementQtyClicked }) => {
  ...
  const nodes = hasProducts ? (
    ...
    products.map(product =>
      <CartProduct
        ...
        onIncrementQtyClicked={() => onIncrementQtyClicked(product.id) }
        onDecrementQtyClicked={() => onDecrementQtyClicked(product.id) }
      />
     )
    }
      <p>Total: &#36;{total}</p>
